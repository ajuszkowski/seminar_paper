...
The Martin-Löf type theory, also known as the \textit{Intuitionistic type theory}\footnote{In terms \textit{intuitionistic type theory}, \textit{intuitionistic logic}, we use the word \textit{intuitionistic} as a synonym for \textit{constructive}.}, is based on the principles of mathematical constructivism, that require explicit definition of the way of "constructing" an object in order to prove its existence. Therefore, an important place in intuitionistic type theory is held by the \textit{inductive types}, which were constructed recursively using a basic type (zero) and successor function which defines "next" element.
...
This section provides some illustrative examples of proofs performed in Coq and Isabelle.
 
 //concider proofs of the de Morgan's laws \ref{tauto_demorgan1} and \ref{tauto_demorgan2}.

// then: nth arith formula + extraction)
