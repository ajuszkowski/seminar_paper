...
% Think: on slide 111 of http://www.seas.upenn.edu/~jean/what-isa-proof.pdf there's said "No de Morgan laws", But we have proved (further) it in Coq without excl middle axiom.

// DIFFERENCES BTW CLASSIC AND INTUITIONISTIC LOGIC
see slide page 101 %http://www.seas.upenn.edu/~jean/what-isa-proof.pdf
(and book by prof.)
...
// in '(*' brackets - comment
\begin{coq}[caption={Definition of basic propositional terms in Coq}]
...
\begin{coq}[caption={Proof of propositional logic tautology in Coq: the de Morgan's law for propositions},label={DeMorganPropositional_Coq}]
...
% outline: https://math.stackexchange.com/a/1635653
\begin{lstlisting}
...
% for Coq: https://github.com/coq/coq/wiki/SquareRootTwo
% for Isabelle: trivial
...
Isabelle has more powerful meta-language syntax to operate tactics: it allows to use the functional combinators of tactics called tacticals for building up complex tactics from simpler ones. Common tacticals perform sequential composition, disjunctive choice, iteration, or goal addressing, which make Isar the Turing-complete language in the domain of logic <TODO: say better (?)>. [REFERENCE: implementaion.pdf in Isabelle Jedit]. In comparison, Coq allows to use only the composition of tactics. Moreover, Isar has more relaxed syntax than Gallina in sense that its proof look more like mathematical proof because of writing the goals during the proof (e.g., '\isabelleinline{assume "\<not> P x" then have "\<exists>x. \<not> P x"}') and using the connectors that can be read conveniently by human (e.g., '\isabelleinline{then}' abbreviates '\isabelleinline{from this}', '\isabelleinline{hence}' expands to '\isabelleinline{then have}', etc.). In contrast, proofs written in Coq look more like programs written in imperative programming language: the sequence of states need to be executed in order to check how the directives (application of tactics) change the state of proof. On the other hand, the brevity of Gallina language may let the experienced user, that knows the syntax and tactics well, spend less time for writing the proof in Coq rather than in Isabelle.
