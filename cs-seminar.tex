...
Excluded middle does not hold in coq...
\begin{lstlisting}[language=coq, caption={Fail of the automatic proof of excluded middle axiom in Coq}, label={ex_mid_auto_fail_coq}]
Lemma ExcludedMiddle_Coq: forall P: Prop,
    P \/ ~P.
Proof.
    try tauto.
Abort.
...
Still, double negation can be proved solely in intuitionistic logic:
...
Nonetheless, Coq's logic can be extended to classical logic by assuming the excluded middle axiom (we use that in the proof ??? of nth number of arith progr)
