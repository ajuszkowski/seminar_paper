...
// TODO: Hilbert system: consistent, incomplete, independent | FOL | HOL | Coq ...
...
\textit{Coq} is a formal proof assistant system which has been developed at INRIA (Paris, France) since 1984. Coq is based on Calculus of Inductive Constructions, which uses inductive and dependent types, and represents an implementation of intuitionistic logic. Nonetheless, Coq's logic may be easily extended into classical logic by assuming the excluded middle axiom~\ref{axiom_excluded_middle}.
...
Therefore, numerous of theorems, such as the classical logic tautology Peirce's law~\eqref{tauto_peirce}, can not be proved in intuitionistic logic, while being valid in classical logic, which makes the latter strictly weaker~\cite{Rush14} (Coq's tactic for automatic reasoning of propositional statements \texttt{tauto} fails to prove this automatically).
...
// as illustrative examples, concider proofs of the de Morgan's laws \ref{tauto_demorgan1} and \ref{tauto_demorgan2}.
...
//The following example~\ref{ex_morgan_propos_coq} shows the proof of De Morgan's law in Coq system. The law statement is being proved for all propositions of type \texttt{Prop}  <which is ...> ... . For arguments of type \texttt{bool} the proof is trivial.
...
todo: define list (or sth inductive/dependent, see in manuals)
...
// ... A practical example: given the sum n natural numbers $S_{n}$ defined recursively, proof the correctness of formula $2 * S_{n} = {n \cdot (n + 1)}$ 
